...
// document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('.cart-img-top').forEach(item => {
    item.addEventListener('click', (e) => {
      window.location.href = '/pages/item_info.php' + '?id=' + e.target.closest('.cart-item').querySelector('.article').textContent.replace(/\D/g, '');
    })
  })
  ...
  document.querySelectorAll('#value').forEach(item => item.addEventListener('change', (e) => {
    ...
    let article = parent.querySelector('.article').textContent.replace(/\D/g, '');
    let maxcount = parent.querySelector('.item_count').textContent.replace(/\D/g, '');
    ...
    document.cookie = 'cartCount' + '=' + encodeURIComponent(count), { path: '/' };
    location.reload();
    ...
    //   },
    //   body: 'inputData=' + encodeURIComponent(e.target.value),
    // })
    // .then(response => {
    //   if (!response.ok) {
